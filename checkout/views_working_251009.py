...
def _finalize_paid(order: Order):
    """
    Mark as paid, decrement inventory once, and send confirmation email exactly once.
    """
    ...
    # Skicka kvitto EN gång (om vi nyss markerade paid och inget kvitto är loggat)
    if just_marked_paid and order.email and not order.confirmation_sent_at:
        if send_order_confirmation(order, customer_email=order.email, customer_name=order.full_name):
